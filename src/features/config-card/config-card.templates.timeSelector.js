...
export function getTimeSelectorHTML(isAnySetLogged) {
  ...
  const optionsHtml = timeOptions
    ...
    .map((opt) => {
      ...
      return `<li data-time="${opt.name}"><div class="item-main-line flex-line-container"><span class="flex-priority text-info">${opt.name}&nbsp;</span><span class="truncate-text data-highlight ${opt.colorClass}">${durationText}</span></div></li>`;
    })
    .join("");
